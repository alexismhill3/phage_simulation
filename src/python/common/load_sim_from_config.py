...
def construct_model_with_genomes(config_file, genomes):
    """
...
def construct_model(config_file):
    """
    ...
    """
    data = _load_conf(config_file)
    genomes = construct_genome(config_file) if "genome" in data else []
    model = construct_model_with_genomes(config_file, genomes)
    return model
